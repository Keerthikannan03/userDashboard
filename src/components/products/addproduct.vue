...
<template>
    ...
    <div>
      <v-form ref="productform" @submit.prevent="saveproduct()">
        <div class="row pb-2 overflow-y-auto overflow-x-hidden border-bottom" style="height: 80vh;">
          <div class="col-md-6">
            <div>
              <div class="p-2">
                <h6>Description</h6>
          ...
          <div class="col-md-6 mt-md-0 mt-3">
            <div>
              ...
              <v-card max-height="320px" class="p-2 overflow-auto">
                <div class="row g-1">
                  <div class="col-lg-3 col-md-6 file-upload">
                    <v-file-upload
                      v-model="prodImage"
                      @change="onFileChange"
                    ...
                    >
                      <template v-slot:title>
                        <div style="font-size: 14px;">
                          <span class="text-info text-decoration-underline">Click to Upload image
                          </span>
                          <span>or Drag and drop</span>
                        </div>
                      </template>
                    </v-file-upload>
                    <div v-if="fileUploadError" class="text-caption mt-1" style="color: #B00020;">
                      {{ fileUploadErrorMsg }}
                    </div>
                  </div>
                  <div v-for="(prodimg, index) in productsData.images" :key="index" class="col-lg-3 col-md-6 product-image position-relative" style="height: 150px;">
                    <v-img :src="prodimg" width="100%" height="100%" class="border border-1 rounded"/>
                      <v-icon @click="deleteProdImg(index)" class="position-absolute top-0 right-0 ma-1 bg-light rounded-5">mdi-close</v-icon>
                  </div>
                  <div class="col-md-3" v-if="productsData.images.length == 0" style="height: 150px;">
              ...
              <v-card class="p-2">
                <div class="row">
                  <v-text-field
                    ...
                    :rules="requiredError" @keypress="filterEvent"
                    label="Items Weight"
                    density="compact"
                    variant="outlined"
                    required suffix="kg"
              ...
              <v-card class="p-2">
                <div class="row">
                  <v-text-field
                    ...
                    density="compact"
                    variant="outlined"
                    required append-inner-icon="mdi-percent"
                    class="col-md-6"
                  ></v-text-field>
                  <v-text-field
                    v-model="productsData.price"
                    :rules="requiredError" @keypress="filterNumbers"
                    density="compact" label="Price"
                    variant="outlined"
                    required append-inner-icon="mdi-currency-inr"
                    class="col-md-6"
                  ></v-text-field>
                </div>
              </v-card>
        ...
        <div class="p-2 text-end">
          ...
          <v-btn @click="saveproduct()" :loading="loadsubmit" :disabled="loadsubmit" type="submit" color="success">Save Product</v-btn>
        </div>
      </v-form>
    </div>
